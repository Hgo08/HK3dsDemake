...
void addText(const std::string& text, u8 fontNum, float x, float y, float z, float scaleX, float scaleY, bool screenTarget, u32 color){
	...
	texts[totalTexts].screenTarget = screenTarget;
...
void textRender(bool screenTarget)
{
	...
	for (u16 i = 0; i < totalTexts; i++) {
		if (texts[i].screenTarget == screenTarget)
